...
const EventsPage = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-card border-b border-border py-3">
        <div className="container flex gap-2 flex-wrap items-center">
          ...
          {(profile?.role === "event_organizer" || profile?.role === "admin") && (
            <button onClick={() => setShowListModal(true)} className="ml-auto px-4 py-1.5 rounded-full text-[13px] font-bold bg-indigo/10 text-indigo border border-indigo hover:bg-indigo/20 transition-all">➕ Create Event</button>
          )}
        </div>
      </div>
      ...
      <ListEventModal open={showListModal} onOpenChange={setShowListModal} />
    </div>
  );
};
