    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-card border-b border-border py-3">
        <div className="container flex gap-2 items-center flex-wrap">
          ...
          {(profile?.role === "vehicle_provider" || profile?.role === "admin") && (
            <button onClick={() => setShowListModal(true)} className="px-4 py-1.5 rounded-full text-[13px] font-bold bg-ruby/10 text-ruby border border-ruby hover:bg-ruby/20 transition-all">➕ List Vehicle</button>
          )}
      ...
      <ListVehicleModal open={showListModal} onOpenChange={setShowListModal} />
    </div>
  );
};
